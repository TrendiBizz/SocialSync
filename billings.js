...
billing.populateBillingHistorySection = function (successCallback) {
    ...
    // billing-section-planfeature.html
    billingHistoryDiv.on("click", "li[data-history-item-id]", function (e) {
        billing.showHistoryInvoice($(e.target).attr("data-history-item-id"));
    });

    // billing-section-plan-totals.html
    billingHistoryDiv.on("click", "[data-switch-to-annual-action]", function () {
        window.billing.switchToAnnual()
    });

    // billing-section-plan-submit.html
    billingHistoryDiv.on("click", "[data-cancel-payment-action]", function () {
        window.billing.cancelPayment();
        return false;
    });
    billingHistoryDiv.on("click", "[data-show-billing-section-action]", function () {
        window.billing.showBillingSection(); 
        return false;
    });
    billingHistoryDiv.on("click", "[data-enhace-plan-action]", function () {
        window.billing.enhancePlanNextHook();
    });

    // billing-section-planfeature-pro.html
    billingHistoryDiv.on("click", "[data-click-update-billing-price-action]", function () {
        window.billing.updatePrice();
    });
    billingHistoryDiv.on("change", "[data-change-update-billing-price-action]", function () {
        window.billing.updatePrice();
    });
...
billing.populateEnhancePlanSection = function (planId, formstash, successCallback) {
    ...
    enhancePlanDiv.on("click", "[data-show-teams-pricing-action]", function() {
        window.plans.showTeamsPricingPopup();
    });
...
billing.populateAllSections = function (planId, updateBilling, formstash, successCallback) {
    ...
    //billing-section-planfeature.html
    $targetDiv.on("click", "[data-show-teams-pricing-action]", function() {
        window.plans.showTeamsPricingPopup();
    });

    // billing-section-plan-totals.html
    $targetDiv.on("click", "[data-switch-to-annual-action]", function () {
        window.billing.switchToAnnual()
    });

    // billing-section-plan-submit.html
    $targetDiv.on("click", "[data-cancel-payment-action]", function () {
        window.billing.cancelPayment();
        return false;
    });
    $targetDiv.on("click", "[data-show-billing-section-action]", function () {
        window.billing.showBillingSection(); 
        return false;
    });
    $targetDiv.on("click", "[data-enhace-plan-action]", function () {
        window.billing.enhancePlanNextHook();
    });

    // billing-section-planfeature-pro.html
    $targetDiv.on("click", "[data-click-update-billing-price-action]", function () {
        window.billing.updatePrice();
    });
    $targetDiv.on("change", "[data-change-update-billing-price-action]", function () {
        window.billing.updatePrice();
    });

    // billing-section-payment.html
    $targetDiv.on("click", "[data-hide-billing-section-action]", function () {
        window.billing.hideBillingSection();
    });
    $targetDiv.on("click", "[data-update-later-confirmation-message]", function (e) {
        if (confirm($(e.target).attr("data-update-later-confirmation-message"))) { 
            document.location = $(e.target).attr("data-update-later-redirect-url");
            return false; 
        }
    });
    $targetDiv.on("click", "[data-cancel-billing-action]", function (e) {
        window.billing.recordBillingCancel(); 
        document.location = $(e.target).attr('data-cancel-billing-action')
        return false;
    });
    $targetDiv.on("click", "[data-start-free-trial-action]", function () {
        window.billing.billingNextHook();
    });
    $targetDiv.on("click", "[data-submit-payment-action]", function () {
        window.billing.submitPayment();
    });
    $targetDiv.on("click", "[data-enhance-plan-action]", function (e) {
        if (!!$(e.target).attr("data-ajax-billing-page")) {
            window.billing.switchProBillingPeriod(); 
            window.billing.confirmationEnhancePlan();
        } else {
            window.billing.switchProBillingPeriod($(e.target).attr('data-switch-pro-url'));
        }
    });
    $targetDiv.on("click", "[data-switch-pro-action]", function (e) {
        billing.switchProBillingPeriod($(e.target).attr('data-redirect')); 
    });
    $targetDiv.on("click", "[data-modify-plan-action]", function (e) {
        if (!!$(e.target).attr("data-ajax-billing-page")) {
            window.billing.confirmationEnhancePlan();
        } else {
            window.parent.location = $(e.target).attr('data-redirect-url');
        }
    });
    $targetDiv.on("click", "[data-redirect-parent-action]", function (e) {
        window.parent.location = $(e.target).attr('data-redirect-url');
    });
    $targetDiv.on("click", "[data-reload-action]", function () {
        window.parent.location= window.parent.location.href;
        return false;
    });

    //signup-form.html
    $targetDiv.on("click", "[data-submit-signup]", function () {
        window.hs.throbberMgrObj.add('._submitSignup'); 
        window.billing.createAccountNextHook();
    });
