...
import { PENDO_TARGETS, updateAsyncDataToPendo } from 'fe-lib-pendo';
...
import { openAppDirectoryRoute } from 'hs-app-directory-dashboard/components/loader';
...
import initCSPSentry from 'sentry/CSPSentry';
...
import initAdvertisingInsights from "apps/advertising-insights/init";
...
dashboard.init = function () {
    ...
    if (darklaunch.isFeatureEnabledOrBeta("PROM_7037_ENABLE_ADS_INSIGHTS")) {
        initAdvertisingInsights(hs.memberId);
    }
    ...
    //Update Pendo data
    hootbus.on(events.INVITE_ORG_MEMBERS_SUCCESS, function () {
        updateAsyncDataToPendo()
    });

    hootbus.on(events.REMOVE_MEMBER_SUCCESS, function () {
        updateAsyncDataToPendo()
    });

    hootbus.on(events.DELETE_ORG_SUCCESS, function () {
        updateAsyncDataToPendo()
    });
    ...
    if (orgSuspendedBanner) {
        ...
        createRoot(orgSuspendedBanner).render(React.createElement(OrgSuspendedBanner, {
...
dashboard.startPingTimer = function () {
    // call it once immediately, then on an interval.
    member.pingServer();
...
dashboard.loadPublisherAjaxCallBack = function (data, callback, bulkScheduleFeature = 1) {
    ...
    if (bulkScheduleFeature === 0) {
        ...
        showBulkScheduleDialogBtn.bind('click', function () {
            ...
            publisher.highlightSidebarBtn('showBulkScheduleDialogBtn')
        });
    }
...
dashboard.loadOrganizations = function (section, params, origin) {
...
window.loadAppDirectory = function (params) {
    // This is a legacy function, during the transition period it loads the new app directory
    var queryParams = params;
    if (typeof queryParams === 'string') {
        queryParams = { sectionKey: params };
    }
    openAppDirectoryRoute(queryParams)
};
...
dashboard.postLoadStreams = function (tabId, callback, forceLoad) {
    ...
    ajaxCall({
        ...
        success: async function (data) {
            ...
            const parentContainer = document.getElementById('_renderQuickSearchIcon');
            if (parentContainer) {
                parentContainer.innerHTML = renderToString(React.createElement(Icon, {
                    fill: 'black',
                    glyph: SearchIcon,
                    size: '17'
                }));
...
dashboard.showLegacyPaywall = function (data, featureAddSuccess) {
    ...
    var fnRenderPopup = function (popupData) {
        ...
        // feature-access-denied-popup/owner/premium-organization.html
        // feature-access-denied-popup/self/premium-organization.html
        $popup.on("click", "[data-upgrade-org-action]", function (event) {
            window.teammanagement.upgradeOrganization($(event.target).attr("data-org-id"), $(event.target).attr("data-current-member-owns-this-org") === "true");
            return false;
        });

        // feature-access-denied-popup/self/premium-organization-share-network.html
        $popup.on("click", "[data-upgrade-plan-action]", function () {
            window.location = window.hs.c.rootUrl+'/billing?planId=2';
            return false;
        });
    };
    ...
    ajaxCall({
        ...
        success: function (data) {
            if (data.userAccountIsDisabled) {
                window.location = hs.util.getUrlRoot() + '/logout?redirect=/logged-out';
                return false;
            }
