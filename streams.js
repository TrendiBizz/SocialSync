...
stream.streamHelper.initAppsSection = function () {
    ...
    ajaxCall({
        ...
        success: function (data) {
            ...
            $box.find('._viewAllApps').click(function () {
                openAppDirectoryRoute();
            }).end()
        }
    }, 'qm');
};
...
stream.streamHelper.init = function (opts) {
    ...
    // Variables below used to dynamically determine correct selectors after migrating to new bento tabs in fe-app-streams
    const tabSelector = $box.find('._tab').length ? '._tab' : 'div[role="tab"]';
    const tabKey = $box.find('._tab').length ? 'tab' : 'key';

    // binds
    $box
        .delegate(tabSelector, 'click keyup', function () {
            var target = $(this).data(tabKey);
            if (target && $box.find('._tabSection._' + target).length && !$box.find('._tabSection._' + target).is(':visible')) {
                ...
                // tab specific
                if (target === 'apps') {
                    if(darklaunch.isFeatureDisabled('NGE_21627_MIGRATE_APPS_TAB')) {
        ...
        // Open app directory for app installs
