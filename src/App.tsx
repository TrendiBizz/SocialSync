...
const PLATFORM_DETAILS: Record<Platform, {
  ...
  linkedin: { name: 'LI', logo: LinkedInLogo, shareType: 'intent', urlBase: 'https://www.linkedin.com/sharing/share-offsite/?url=', dialClass: 'linkedin-dial-button' }, 
...
function App() {
  console.log("--- SocialSync App Component Rendering --- LOCAL_DEV_TEST_FULL_UI_V1"); // Added test log
        ...
        window.FB.getLoginStatus(function(response: any) { 
          if (response.status === 'connected') {
            ...
            window.FB.ui({
              ...
              if (shareResponse && !shareResponse.error_message) { console.log('Facebook SDK: Posting complete.', shareResponse); } 
              else { console.error('Facebook SDK: Error while posting.', shareResponse ? shareResponse.error_message : 'Unknown error'); alert('Facebook sharing error: ' + (shareResponse ? shareResponse.error_message : 'Could not share.')); }
            });
          } else {
            ...
            window.FB.login(function(loginResponse: any) {
              if (loginResponse.authResponse) {
                ...
                function(shareResponseAfterLogin: any){ 
                  if (shareResponseAfterLogin && !shareResponseAfterLogin.error_message) { console.log('Facebook SDK: Posting complete after login.', shareResponseAfterLogin); } 
                  else { console.error('Facebook SDK: Error while posting after login.', shareResponseAfterLogin ? shareResponseAfterLogin.error_message : 'Unknown error'); alert('Facebook sharing error after login: ' + (shareResponseAfterLogin ? shareResponseAfterLogin.error_message : 'Could not share.')); }
                });
              } else { console.log('Facebook SDK: User cancelled login or did not fully authorize.'); alert('Facebook login was cancelled or not fully authorized.'); }
            }, {scope: 'public_profile'}); 
          }
        });
      } else { console.error('Facebook SDK not loaded!'); alert('Facebook features are currently unavailable. Please try again later.'); }
